...
namespace CompositeCanvas
{
    internal class ObjectRepository<T> where T : Object
    {
        ...
        public ObjectRepository(string name, Action<T> onRelease = null)
        {
            _name = name;
            if (onRelease == null)
            {
        ...
        {
            // Find existing entry.
            Profiler.BeginSample("(CCR)[ObjectRepository] Valid > Find existing entry");
            for (var i = 0; i < _cache.Count; ++i)
            {
                var entry = _cache[i];
        ...
        public void Release(ref T obj)
        {
            ...
            for (var i = 0; i < _cache.Count; i++)
            {
                ...
                {
                    Profiler.BeginSample("(CCR)[ObjectRepository] Release > RemoveAt");
